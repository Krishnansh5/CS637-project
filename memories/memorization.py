...
class memorization :
    ...
    def __init__(self, source_dir = None, saving_dir = None):
        ...
        if self.source_dir:
            ...
            print("time taken in intializing data container: ", t1 - t0)
    ...
    def create_dist_matrix(self,saveToDisk):
        ...
        for name in self.data_container.keys():
            ...
            self.data_container[name]["dist"] = all_distances
        for name1 in self.data_container.keys():
            for name2 in self.data_container.keys():
    ...
    def learn_memories_with_fast_CLARANS(self, num_clusters=2, save_to_disk=True):
        ...
        start = time.time()
        cost, medoids, results = fast_clarans.py_fast_clarans(dist_matrix, self.num_samples, num_clusters, 2, 0.025, 123456789)
        end = time.time()
        print("Time taken to build memories: ", end - start)
